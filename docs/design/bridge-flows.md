...
physical L2 domain share the bindings learned through the CDN GR.

The default and UDN external logical router ports join one per-node OVN
`MAC_Binding_Scope`. The scope is symmetric: a binding learned through any
member is available to every member, including the lookup that releases a
packet buffered during a cold miss. In practice, the bridge steering rules
deliver external ARP and NDP to the default GR, so it supplies the learned
bindings. UDNs using a separate uplink do not join the scope because those GRs
belong to a different L2 domain and must learn their own neighbors.
