...
func TestController_reconcile(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:      "reconciles pod RouteAdvertisement for DPU host default network in no-overlay mode with next-hop raw config",
			ra:        &testRA{Name: "ra", AdvertisePods: true, SelectsDefault: true},
			transport: types.NetworkTransportNoOverlay,
			frrConfigs: []*testFRRConfig{
				{
			...
			nodes: []*testNode{
				{
					Name:                      "node",
					Labels:                    map[string]string{dpuHostNodeLabel: ""},
			...
			expectFRRConfigs: []*testFRRConfig{
				{
					...
					Annotations:       map[string]string{types.OvnRouteAdvertisementsKey: "ra/frrConfig/node"},
					NodeSelector:      map[string]string{"kubernetes.io/hostname": "node"},
					RawConfigPriority: dpuHostNextHopRawConfigPriority,
					RawConfig: `route-map 1.0.0.100-out permit 1
 set ip next-hop 172.18.255.254
exit
`,
					Routers: []*testRouter{
						{ASN: 1, Prefixes: []string{"1.1.0.0/24"}, Neighbors: []*testNeighbor{
