...
func (udng *UserDefinedNetworkGateway) AddNetwork() error {
	...
	if udng.Uplink() == "" {
		...
		err = udng.uplinkGatewayController.startNetwork(
			udng.NetInfo,
			udng,
			func(resolved *resolvedUplinkGateway) error {
				if resolved == nil {
					return fmt.Errorf("waiting for resolved Uplink gateway configuration for network %s",
						udng.GetNetworkName())
				}
				// addNetworkWithResolvedUplink can fail after creating only part
				// of the gateway. Mark it as programmed immediately before setup
				// so StartNetwork invokes the idempotent teardown path only when
				// programming was actually attempted.
...
func (udng *UserDefinedNetworkGateway) withdrawUplinkGateway() error {
	...
	if err := udng.delNetwork(); err != nil {
		return fmt.Errorf("failed to withdraw Uplink gateway for network %s: %w", udng.GetNetworkName(), err)
	}
