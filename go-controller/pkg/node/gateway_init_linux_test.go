...
func shareGatewayInterfaceTest(app *cli.App, testNS ns.NetNS,
	...
	app.Action = func(ctx *cli.Context) error {
		...
		fexec.AddFakeCmdsNoOutputNoError([]string{
			"ovs-ofctl add-flow breth0 cookie=0xdeff105, priority=100, in_port=5, ip, actions=ct(commit, zone=64000, exec(load:0x1->NXM_NX_CT_LABEL)), output:7",
			"ovs-ofctl add-flow breth0 cookie=0xdeff105, priority=100, in_port=LOCAL, ip, actions=ct(commit, zone=64000, exec(load:0x2->NXM_NX_CT_LABEL)), output:7",
			...
			"ovs-ofctl add-flow breth0 cookie=0xdeff105, priority=100, table=1, ct_label=0x1, actions=output:5",
			"ovs-ofctl add-flow breth0 cookie=0xdeff105, priority=100, table=1, ct_label=0x2, actions=LOCAL",
			"ovs-ofctl add-flow breth0 cookie=0xdeff105, priority=2, table=1, tcp, ip_dst=192.168.1.10, tp_dst=6443, actions=LOCAL",
			"ovs-ofctl add-flow breth0 cookie=0xdeff105, priority=2, table=1, tcp, ip_dst=192.168.1.10, tp_dst=2379, actions=LOCAL",
			"ovs-ofctl add-flow breth0 cookie=0xdeff105, priority=2, table=1, tcp, ip_dst=192.168.1.10, tp_dst=2380, actions=LOCAL",
