...
func (b *BridgeConfiguration) commonFlows(hostSubnets []*net.IPNet) ([]string, error) {
	...
	if ofPortPhys != "" {
		...
		if util.IsNetworkSegmentationSupportEnabled() {
			dftFlows = append(dftFlows, b.neighborDiscoverySteeringFlows(ofPortPhys, matchVLAN)...)
		}
	}
	...
	if ofPortPhys != "" {
		...
		if config.IPv6Mode {
			...
			// Separate-uplink bridges have no default network port and retain
			// explicit delivery to their no-flood UDN patch ports.
			patchActions := ""
			if !hasDefaultNetConfig {
				patchActions = b.patchOutputActions()
			}
			for _, icmpType := range []int{types.RouteAdvertisementICMPType, types.NeighborAdvertisementICMPType} {
				dftFlows = append(dftFlows,
					fmt.Sprintf("cookie=%s, priority=14, table=1,icmp6,icmpv6_type=%d actions=%sFLOOD",
						nodetypes.DefaultOpenFlowCookie, icmpType, patchActions))
			}
			if hasDefaultNetConfig {
...
// neighborDiscoverySteeringFlows prevents ARP and NDP packets from reaching
// CUDN GR patch ports.
//
...
// NS, the priority-0 NORMAL action floods to all ports. CUDN patch ports are
// configured with OVS no-flood so NORMAL does not deliver flooded traffic to
// them.
//
// All external ARP and NDP control traffic is sent only to the default-network
// GR patch and NORMAL. The default GR owns the shared MAC bindings used by the
// CUDN GRs, while NORMAL preserves FDB learning and host delivery.
//
// Must be called with bridge.mutex held.
func (b *BridgeConfiguration) neighborDiscoverySteeringFlows(ofPortPhys, matchVLAN string) []string {
	...
	actions := fmt.Sprintf("output:%s,NORMAL", defaultNetConfig.OfPortPatch)
	if config.IPv4Mode {
		flows = append(flows,
			fmt.Sprintf("cookie=%s, priority=45, table=0, in_port=%s, %s arp, actions=%s",
	...
	if config.IPv6Mode {
		for _, icmpType := range []int{
			...
			flows = append(flows,
				fmt.Sprintf("cookie=%s, priority=52, table=0, in_port=%s, %s icmp6, icmpv6_type=%d, actions=%s",
					nodetypes.DefaultOpenFlowCookie, ofPortPhys, matchVLAN, icmpType, actions))
		}
	}
...
// hasCUDNPatchPorts returns true if any non-default network has its patch port
// set. Must be called with bridge.mutex held.
...
// patchOutputActions returns explicit output actions for every patched network
// port. Must be called with bridge.mutex held.
func (b *BridgeConfiguration) patchOutputActions() string {
	var actions string
	for _, netConfig := range b.patchedNetConfigs() {
		actions += "output:" + netConfig.OfPortPatch + ","
	}
