...
func (ovn *Controller) syncServices(services []interface{}) {
	...
	for _, serviceInterface := range services {
		...
		for _, svcPort := range service.Spec.Ports {
				...
				if err == nil {
					for _, gatewayRouter := range gatewayRouters {
						...
						for _, physicalIP := range physicalIPs {
							if err != nil {
								klog.Warningf("Service Sync: Gateway router %s does not have physical ip (%v)",
									gatewayRouter, err)
								continue
							}
							svcRejectACLs[ovn.generateACLName(lb, physicalIP, svcPort.NodePort)][lb] = hasEndpoints
						}
					}
				}
			}
			...
			if err != nil {
				...
				svcRejectACLs[ovn.generateACLName(lb, service.Spec.ClusterIP, svcPort.Port)][lb] = hasEndpoints
			}
			for _, extIP := range service.Spec.ExternalIPs {
				...
				for _, gateway := range gateways {
					...
					svcRejectACLs[ovn.generateACLName(lb, extIP, svcPort.Port)][lb] = hasEndpoints
				}
			}
		}
	}
	...
	if err != nil {
		...
		if err := json.Unmarshal([]byte(data), &x); err != nil {
			...
			klog.Infof("No reject ACLs currently configured in OVN")
		} else {
			for _, entry := range x.Data {
				// ACL entry format is a slice of: aclName, then another embedded slice of "_uuid": <uuid>
				if len(entry) != 2 {
					continue
				}
				name := entry[0].(string)
				uuidData := entry[1].([]interface{})
				if len(uuidData) != 2 {
					continue
				}
				uuid := uuidData[1].(string)
