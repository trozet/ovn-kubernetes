...
func (oc *Controller) addLogicalPort(pod *kapi.Pod) (err error) {
	...
	if err == nil {
		...
		if err = oc.lsManager.AllocateIPs(logicalSwitch, podIfAddrs); err != nil && err != ipallocator.ErrAllocated {
			...
			releaseIPs = true
			klog.Warningf("Unable to allocate IPs for already annotated pod: %s, IPs: %s, error: %v", pod.Name,
				util.JoinIPNetIPs(podIfAddrs, " "), err)
		} else {
			needsIP = false
		}
	}
	...
	if needsIP {
		...
		// ensure we have/can allocate the IP found in OVN
		if len(podIfAddrs) == 0 {
